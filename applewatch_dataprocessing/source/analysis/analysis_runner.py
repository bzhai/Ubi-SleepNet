...
def figures_leave_one_out_three_class_performance(result_path):
    attributed_classifier = AttributedClassifier(name='Neural Net',
                                                 classifier=MLPClassifier(activation='relu',
                                                                          hidden_layer_sizes=(15, 15, 15),
                                                                          max_iter=1000, alpha=0.01, solver='lbfgs'))
...
result_path = Path(datetime.now().strftime("%Y%m%d-%H%M%S"))
if not os.path.exists(result_path):
    Path(result_path).mkdir(parents=True, exist_ok=True)
result_path = result_path.absolute()
print(f"Results saved to {result_path}")
figures_leave_one_out_three_class_performance(result_path)
