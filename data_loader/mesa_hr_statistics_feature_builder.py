...
class MesaHRStatisticFeatureBuilder(object):
    def __init__(self, cfg, hrv_win, standarize):
        ...
        self.output_path = cfg.MESA_ACC_HR_STATISTIC_CSV_ALIGNED  # save the extracted Act and HR statistic feats here
    ...
    def process_all_files(self, is_test=False, parallel=False):
        '''
        ...
        if not os.path.exists(self.processed_records):
            with open(self.processed_records, "w") as output:
                ...
                writer.writerows(self.total_processed)
        # tag = datetime.now().strftime("%Y%m%d-%H%M%S")
        if parallel:
            batch_size = 35
            idx = 660
            print(r"parallel process start %s" %datetime.now().strftime("%Y%m%d-%H%M%S"))
            while idx < len(total_subjects_list):

                if idx + batch_size<len(total_subjects_list):
                    pids = total_subjects_list[idx:(idx+batch_size)]
                else:
                    pids = total_subjects_list[idx:]
                print(f"start end pid:{pids[0]}  {pids[-1]}")
                with futures.ProcessPoolExecutor(max_workers=numexpr.detect_number_of_cores()-2) as pool:
                    for pid in pids:
                        t1 = pool.submit(self.process_extract_features, pid, all_acc_files, all_hr_files, overlap_df)
                    idx += batch_size
                # time.sleep(65.0)
                # idx += batch_size
        else:
            for pid in total_subjects_list:
                self.process_extract_features(pid, all_acc_files, all_hr_files, overlap_df)

    def process_extract_features(self, pid, all_acc_files, all_hr_files, overlap_df):
        processed_details = []
        mesa_id = "%04d" % pid
        ...
        if len(acc_inlist_idx) > 0 and len(hr_inlist_idx) > 0:
            ...
            for _, hr_epoch_idx in enumerate(list(hr_epoch_set)):
                ...
                hr_epoch = tmp_hr_df['HR'].values
                all_hr_features = {}

                # calculate the hr statistic features
                hr_statistic = get_hr_statistic_feature(hr_epoch)
                all_hr_features.update(hr_statistic)
        ...
        if len(feature_list) > 0:
            hr_statistic_acc_df = pd.DataFrame(feature_list)
            hr_statistic_acc_df = hr_statistic_acc_df.reset_index(drop=True)
            combined_pd = hr_statistic_acc_df
            ...
            std_feature = ["activity", "min_hr", "max_hr", "mean_hr", "skw_hr", "kurt_hr", "std_hr"]
            if self.standarize:
            ...
            #     text_file.writelines("ID: {}, successed process \n".format(mesa_id))
            # processed_details.append("ID: {}, successed process".format(mesa_id))
        else:
            print("Acc is empty or HRV is empty!")
            # processed_details.append("ID: {}, failed process".format(mesa_id))
            # with open(self.processed_records, "a") as text_file:
            #     text_file.writelines("ID: {}, failed process".format(mesa_id))
        return processed_details


if __name__ == "__main__":
    config = Config()
    builder = MesaHRStatisticFeatureBuilder(config, hrv_win=0, standarize=False)
    builder.process_all_files(is_test=False, parallel=True)
    print("all processed!")
